...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(() => {
            ...
            // ip_item_name = $(".ip-item-name");
            // ip_item_name.keyup(function () {
            //     let search_string = $('#ip_item_name').val();
            //     let filtered_data = itemList.filter(function (item) {
            //         // return `${item.name}`.toLowerCase().includes(search_string) == 0;
            //         return `${item.name}`.toLowerCase().indexOf(search_string) == 0;
            //     })
            //     console.log(filtered_data);
            //     if (filtered_data.length > 0) {
            //         let style = `position: absolute;z-index:1001;`
            //         let html = `<div class="row items-list" style=${style}>
            //                         <div class="col-md-4" style="width: 100%; text-align: right;height:150px;overflow-y: scroll;">
            //                             <div style="padding:0 0 0 1rem;" id="showItem">`;
            //         filtered_data.forEach((data) => {
            //             html += `<a class="row text-decoration-none" style="color: #000000;" href="#" id="${data.id}">${data.name}</a>`;
            //         })
            //         html += `</div>
            //                 </div>
            //              </div>`;
            ...
            $(window).keydown(function (event) {
                ...
                if (event.keyCode == 9 && activeElement.id == "ip_discount") {
                    addInvoiceDetail();
                    calculate_total();
                }
            });
            ...
            const addInvoiceDetail = () => {
                ...
                document.querySelector('.ip-item-id').focus();
            }
            const clearInput = () => {
                $(".ip-item-id").val("");
                $(".ip-item-name").val("");
